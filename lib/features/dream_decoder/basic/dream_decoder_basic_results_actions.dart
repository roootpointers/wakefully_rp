...
class DreamDecoderBasicResultsActions extends StatelessWidget {
  final String journeyId;

  const DreamDecoderBasicResultsActions({super.key, required this.journeyId});

  @override
  Widget build(BuildContext context) {
    return Column(
      children: [
        _isSubscribed(context) ? Container() : const _ResultsPremiumUpsell(),
        SizedBox(
          height: 56,
          ...
          child: TextButton(
            onPressed: () {
              ...
              subsribed || journeysBloc.state.blobs.isEmpty
                  ? dreamsBloc.add(DreamsEvent.dreamSaved(journeyId))
                  : _showSaveConfirmation(context, journeyId);
            },
